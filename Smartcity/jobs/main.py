...
SANTA_CLARA_COORDINATES = {"latitude": 37.3541, "longitude": -121.9552}

# Calculate movement increments
LATITUDE_INCREMENT = (SANTA_CLARA_COORDINATES['latitude'] - SAN_JOSE_COORDINATES['latitude']) / 100
LONGITUDE_INCREMENT = (SANTA_CLARA_COORDINATES['longitude'] - SAN_JOSE_COORDINATES['longitude']) / 100
...
start_location = SAN_JOSE_COORDINATES.copy()
...
def generate_weather_data(vehicle_id, timestamp, location):
    return {
        ...
        'humidity': random.randint(20, 80),  # percentage
        'airQualityIndex': random.uniform(10, 150)  # AQI Value
    }
...
def simulate_vehicle_movement():
    """
    Simulates the movement of a vehicle towards Santa Clara by 
...
def simulate_journey(producer, vehicle_id):
    while True:
        ...
        if (vehicle_data['location'][0] >= SANTA_CLARA_COORDINATES['latitude']
                and vehicle_data['location'][1] <= SANTA_CLARA_COORDINATES['longitude']):
            print('Vehicle has reached Santa Clara. Simulation ending...')
            break
